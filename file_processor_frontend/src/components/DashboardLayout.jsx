    ...
    <div className="min-h-screen bg-[#FAFAFA] flex font-sans text-zinc-900 selection:bg-indigo-100 selection:text-indigo-900">
      ...
      >
        ...
        <div className="p-4 mt-auto mb-4 px-4 space-y-1">
           <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest px-3 mb-3">Project & Contact</h3>
  ...
  return (
      <div className="w-full flex flex-col pt-4 pb-12 w-full mx-auto animate-in fade-in zoom-in-95 duration-500">
         <header className="mb-12 text-center lg:text-left flex flex-col md:flex-row justify-between items-end gap-6">
            <div>
                <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-zinc-900 drop-shadow-sm mb-3">Welcome to DocuMind.</h1>
                <p className="text-zinc-500 text-lg font-medium max-w-xl">Supercharge your files with native AI. Select a workflow below to get started.</p>
            </div>
         </header>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            ...
            <FeatureCard onClick={() => onNavigate('audio-transcriber')} title="Audio Transcribe" desc="Convert large meeting recordings or podcasts to raw text using Whisper AI." icon={tools['audio-transcriber'].icon} color="purple" />
         ...
         <div className="flex items-center gap-4 mb-6">
            <h2 className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Coming Soon in v2.0</h2>
            <div className="h-px bg-gradient-to-r from-zinc-200 to-transparent flex-1"></div>
         </div>
...
function FeatureCard({ onClick, title, desc, icon, color }) {
    const colorMap = {
      indigo: { bg: 'bg-indigo-50', text: 'text-indigo-600', ring: 'ring-indigo-100', hoverBg: 'group-hover:bg-indigo-100', glow: 'bg-indigo-500' },
      purple: { bg: 'bg-purple-50', text: 'text-purple-600', ring: 'ring-purple-100', hoverBg: 'group-hover:bg-purple-100', glow: 'bg-purple-500' },
      blue:   { bg: 'bg-blue-50', text: 'text-blue-600', ring: 'ring-blue-100', hoverBg: 'group-hover:bg-blue-100', glow: 'bg-blue-500' },
    };
    const c = colorMap[color];

    return (
        <button onClick={onClick} className="text-left group relative p-8 bg-white rounded-[2rem] shadow-sm ring-1 ring-zinc-200 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] hover:ring-indigo-300 transition-all duration-300 hover:-translate-y-1.5 overflow-hidden flex flex-col h-full focus:outline-none focus:ring-2 focus:ring-indigo-500">
           <div className={`absolute top-0 right-0 p-8 mix-blend-multiply opacity-0 group-hover:opacity-[0.03] transition-opacity blur-2xl rounded-full ${c.glow} w-48 h-48 -mr-16 -mt-16`}></div>
           
           <div className={`w-16 h-16 rounded-[1.2rem] ${c.bg} ${c.text} ${c.ring} flex items-center justify-center mb-8 ${c.hoverBg} group-hover:scale-110 transition-all duration-300 shadow-sm ring-1`}>
               {icon}
           </div>
           
           <h3 className="text-xl font-extrabold text-zinc-900 mb-3 group-hover:text-indigo-900 transition-colors drop-shadow-sm">{title}</h3>
           <p className="text-sm text-zinc-500 font-medium leading-relaxed flex-1">{desc}</p>
           
           <div className="mt-8 flex items-center text-sm font-bold text-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity">
...
function NavItem({ icon, label, active, onClick, isLink }) {
  return (
    <button onClick={onClick} className={`w-full group flex items-center justify-center lg:justify-start gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${active ? 'bg-indigo-50 text-indigo-700 shadow-sm ring-1 ring-indigo-100' : 'text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 hover:shadow-sm ring-1 ring-transparent hover:ring-zinc-200/50'}`}>
      ...
      {isLink && <div className="ml-auto hidden lg:block opacity-0 group-hover:opacity-100 transition-opacity"><svg className="w-3.5 h-3.5 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg></div>}
    </button>
  );
}
